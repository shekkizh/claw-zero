...
def test_flush_trigger_thresholds():
    ...
    assert should_run_memory_flush(
        state, current_tokens=120_000, context_window=200_000, auto_compact_token_limit=160_000
    ) is False
    assert should_run_memory_flush(
        state, current_tokens=130_000, context_window=200_000, auto_compact_token_limit=160_000
    ) is True
...
def test_flush_dedup_within_compaction_cycle():
    ...
    assert should_run_memory_flush(state, current_tokens=90_000, context_window=200_000) is True
...
def test_maybe_flush_respects_trigger(tmp_path, monkeypatch):
    ...
    ran = asyncio.run(maybe_flush_memory(
        model="m", messages=[], memory_store=store, state=state,
        current_tokens=90_000, context_window=200_000,
    ))
