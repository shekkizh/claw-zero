...
- **Inner loop** (`inner_loop.py`) — one activation → one delivered message. It
  flushes memory if triggered, calls the model, runs any tool calls (`bash`,
...
```
  ...
  bus ──msg──▶  │  inbox.receive → append → inner_loop.run(activation) → deliver(reply)→ bus  │ ──msg──▶ bus
                └──────────────────────────────┬─────────────────────────────────────────────┘
                                               │  one activation:
                                  flush? → llm.call → [bash | send_message | spawn_agent]* → compact? → reply Message
```
...
├── config.py              # ClawZeroConfig (roster + spawn knobs; no effort knob)
...
├── tools/                 # bash.py, send_message.py, spawn_agent.py, registry.py (build_tools split)
